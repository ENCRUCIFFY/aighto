...
export default function Chat({ user }) {
  ...
  async function sendMessage(e) {
    ...
    const msg = {
      text, uid:user.uid, username:user.displayName||user.email,
      photoURL:myData.photoURL||null, createdAt:serverTimestamp(),
  ...
  const getMutualFriends = uid => {
    const theirFriends = users.find(u => u.uid === uid)?.friends || [];
    ...
    <div style={{ width:'100vw', height:'100vh', display:'flex', flexDirection:'column', background:'var(--bg)', fontFamily:'var(--font)' }}
        ...
        <div style={{ display:'flex', WebkitAppRegion:'no-drag' }}>
          ...
          {[
            ...
            <button key={btn.action} onClick={() => window.electron?.[btn.action]()}
              onMouseEnter={e => { e.currentTarget.style.background = btn.danger ? '#e74c3c' : 'rgba(255,255,255,0.08)'; e.currentTarget.style.color = 'rgba(255,255,255,0.95)'; }}
              onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = 'rgba(255,255,255,0.4)'; }}
              style={{ background:'transparent', border:'none', cursor:'pointer', width:'46px', height:'38px',
                color:'rgba(255,255,255,0.4)', fontSize:btn.size, display:'flex', alignItems:'center',
                justifyContent:'center', transition:'background 0.1s, color 0.1s', fontFamily:'var(--font)', lineHeight:1 }}>
              {btn.label}
            </button>
          ))}
        </div>
      ...
      {updateStatus === 'downloaded' && (
        <div style={{ background:`linear-gradient(135deg, var(--accent), var(--accent2))`, padding:'8px 16px',
          ...
          <span style={{ fontSize:'0.8rem', color:'white', fontWeight:500 }}>✨ A new version of Aighto is ready!</span>
      ...
      <div style={{ flex:1, display:'flex', overflow:'hidden' }}>
          ...
          <div style={{ padding:'10px', borderTop:'1px solid var(--border)', position:'relative' }}>
            <div style={{ display:'flex', alignItems:'center', gap:'7px' }}>
              ...
              <div style={{ flex:1, overflow:'hidden' }}>
                ...
                <div style={{ fontSize:'0.62rem', color:STATUSES.find(s=>s.value===myStatus)?.color, cursor:'pointer', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}
                  onClick={e => { e.stopPropagation(); setShowStatusMenu(!showStatusMenu); }}>
                  {myData.customStatus || STATUSES.find(s=>s.value===myStatus)?.label}
                </div>
              </div>
              <div style={{ display:'flex', gap:'2px' }}>
                ...
                <button onClick={e => { e.stopPropagation(); setShowSettings(true); }} title="Settings"
        ...
        <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden' }}>
          ...
          <div style={{ height:'52px', flexShrink:0, borderBottom:'1px solid var(--border)', display:'flex', alignItems:'center', padding:'0 16px', gap:'10px' }}>
            ...
            <span style={{ fontFamily:'var(--font-head)', fontWeight:700, fontSize:'1rem', color:'var(--text)', flex:1 }}>{chatTitle}</span>

            {/* Search toggle */}
            <button onClick={() => { setShowSearch(!showSearch); if(showSearch) setSearchQuery(''); }}
              style={{ background:showSearch?`${theme['--accent']}22`:'none', border:showSearch?`1px solid ${theme['--accent']}44`:'1px solid transparent',
                borderRadius:'8px', padding:'5px 10px', cursor:'pointer', color:showSearch?'var(--accent)':'var(--text3)',
                fontSize:'0.82rem', fontFamily:'var(--font)', transition:'all 0.15s' }}>
              🔍 Search
            </button>
          </div>
          ...
          {showSearch && (
            <div style={{ padding:'8px 16px', borderBottom:'1px solid var(--border)', background:'var(--bg2)' }}>
              <input autoFocus type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
                ...
                style={{ width:'100%', background:'var(--bg3)', border:`1px solid ${theme['--accent']}44`,
                  borderRadius:'10px', padding:'9px 14px', color:'var(--text)', fontSize:'0.85rem',
                  fontFamily:'var(--font)', outline:'none' }} />
              {searchQuery && (
                <div style={{ fontSize:'0.72rem', color:'var(--text3)', marginTop:'5px' }}>
          ...
          <div ref={messagesAreaRef} onScroll={handleScroll}
            ...
            {filteredMessages.length === 0 && (
              <div style={{ textAlign:'center', margin:'auto', color:'var(--text3)', fontSize:'0.85rem' }}>
                ...
                {searchQuery ? `No messages found for "${searchQuery}"` : view==='channel' ? `Start the conversation in #${activeChannel}` : `Say hey to ${activeDM?.username}!`}
              </div>
            )}
            {filteredMessages.map((msg, i) => {
              ...
              return (
                <div key={msg.id}
                  ...
                  style={{ display:'flex', flexDirection:isMe?'row-reverse':'row', alignItems:'flex-end', gap:'8px',
                    marginTop:grouped?'2px':'10px', position:'relative', animation:'msg-in 0.2s ease',
                    background:isHighlighted?`${theme['--accent']}10`:'transparent',
                    borderRadius:'10px', padding:isHighlighted?'4px 6px':'0' }}>
          ...
          {showJumpBtn && (
            <button onClick={jumpToBottom}
              style={{ position:'absolute', bottom:'90px', right:'210px', zIndex:50,
                background:`linear-gradient(135deg, var(--accent), var(--accent2))`,
                border:'none', borderRadius:'20px', padding:'7px 16px', color:'white',
                fontSize:'0.78rem', fontWeight:600, cursor:'pointer', fontFamily:'var(--font)',
                boxShadow:`0 4px 16px ${theme['--accent']}44`, animation:'fadein 0.2s ease' }}>
              ↓ Jump to bottom
            </button>
          )}
          ...
          <form onSubmit={sendMessage} style={{ padding:'10px 16px 14px', borderTop:'1px solid var(--border)', display:'flex', gap:'8px', alignItems:'center', position:'relative' }}>
            <div style={{ position:'relative' }}>
              ...
              {showEmojiPicker && (
                <div onClick={e => e.stopPropagation()} style={{ position:'absolute', bottom:'100%', left:0,
              ...
              {users.filter(u=>!u.online||u.status==='offline').map(u => (
                <div key={u.uid} style={{ display:'flex', alignItems:'center', gap:'7px', padding:'4px 6px', opacity:0.4, marginBottom:'2px' }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize:'0.6rem', color:'var(--text3)' }}>Last seen {formatLastSeen(u.lastSeen)}</div>
                  </div>
                </div>
              ))}
            </>
          ...
          <div onClick={e => e.stopPropagation()} style={{ background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:'20px', padding:'28px', width:'280px', boxShadow:'0 20px 60px rgba(0,0,0,0.6)', animation:'fadein 0.2s ease' }}>
            <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:'10px', marginBottom:'16px' }}>
              ...
              <div style={{ textAlign:'center' }}>
                ...
                {/* Last seen */}
                {(!profileUser.online || profileUser.status === 'offline') && profileUser.lastSeen && (
                  <div style={{ fontSize:'0.68rem', color:'var(--text3)', marginTop:'3px' }}>
                ...
                {getMutualFriends(profileUser.uid).length > 0 && (
                  <div style={{ fontSize:'0.7rem', color:'var(--text3)', marginTop:'8px' }}>
                    👥 {getMutualFriends(profileUser.uid).length} mutual friend{getMutualFriends(profileUser.uid).length>1?'s':''}: {getMutualFriends(profileUser.uid).slice(0,2).join(', ')}{getMutualFriends(profileUser.uid).length>2?'...':''}
                  </div>
                )}
              </div>
            </div>
